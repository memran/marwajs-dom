...
function toArray<T>(x: ArrayLike<T> | T | null | undefined): T[] {
  ...
  if (typeof (x as any).length === "number") return Array.prototype.slice.call(x);
  return [x as T];
}
...
function setStyle(el: Element, k: string, v: StyleValue) {
  ...
  (style as any)[camel(k)] = typeof v === "number" && !/^(opacity|zIndex)$/i.test(k) ? `${v}px` : v;
}

/** Tiny internal event registry for `off()` without keeping user closures around */
const REG = new WeakMap<Element | Document | Window, Map<string, Set<EventListenerOrEventListenerObject>>>();
...
export class Dom {
  ...
  constructor(input?: DomInput, root: Document | Element = document) {
    if (!input) {
      ...
      this.list = input.trim() === "" ? [] : Array.from(root.querySelectorAll(input));
  ...
  find(sel: string): Dom {
    const found: Element[] = [];
    this.each(el => found.push(...el.querySelectorAll(sel)));
  ...
  near(sel: string): Dom {
    ...
    this.each(el => {
      ...
      if (!parent) return;
      out.push(...Array.from(parent.children).filter(x => x !== el && x.matches(sel)));
    });
    return new Dom(out);
  }
  ...
  class(token: string): this {
    const mode = token[0] === "!" ? "remove" : token[0] === "?" ? "toggle" : "add";
  ...
  flip(show?: boolean): this {
    return show === undefined ? this.each(el => ((el as HTMLElement).hidden = !(el as HTMLElement).hidden))
                              : show ? this.show() : this.hide();
  }

  // ===== Tree ops =====
  add(child: DomInput | Node): this {
    const nodes = child instanceof Dom ? child.list : (isElement(child) || child === document || child === window)
      ? [child as any]
  ...
  pre(child: DomInput | Node): this {
    const nodes = child instanceof Dom ? child.list : (isElement(child) || child instanceof Node)
      ? [child as any]
      : new Dom(child as any).list;

    return this.each(el => nodes.forEach(n => el.insertBefore(n as any, el.firstChild)));
  }

  before(node: DomInput | Node): this {
    const nodes = node instanceof Dom ? node.list : (isElement(node) || node instanceof Node)
      ? [node as any]
      : new Dom(node as any).list;
    return this.each(el => nodes.forEach(n => el.parentNode?.insertBefore(n as any, el)));
  }
  ...
  after(node: DomInput | Node): this {
    const nodes = node instanceof Dom ? node.list : (isElement(node) || node instanceof Node)
      ? [node as any]
      : new Dom(node as any).list;
    return this.each(el => nodes.forEach(n => el.parentNode?.insertBefore(n as any, el.nextSibling)));
  }
  ...
  off<K extends keyof EventMap>(type?: K, handler?: (ev: EventMap[K]) => void): this {
    this.list.forEach(t => {
      ...
      if (!type) {
        ...
        if (!m) return;
        m.forEach((set, typ) => set.forEach(h => target.removeEventListener(typ, h as any)));
        REG.delete(target);
        return;
      }
      ...
      if (handler) {
        ...
        const set = regGetKey(target, type as string);
        set.forEach(h => target.removeEventListener(type as string, h as any));
        set.clear();
      }
    });
    return this;
  ...
  onD<K extends keyof EventMap>(type: K, sel: string, handler: (ev: EventMap[K], match: Element) => void, opts?: OnOptions): this {
    return this.on(type, (ev: Event) => {
      const t = ev.target as Element | null;
      if (!t) return;
      const host = this.first as Element | Document | Window | undefined;
      const root = (host && "contains" in (host as any)) ? (host as Element) : document;
      const match = (t.closest as any)?.(sel);
      if (match && (root as Element).contains(match)) handler(ev as any, match);
    }, opts);
  }
